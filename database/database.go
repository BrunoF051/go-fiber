...
func ConnectDb() {
	...
	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s  sslmode=disable TimeZone=Europe/Rome", config.Config("PGHOST"), config.Config("PGUSER"), config.Config("PGPASSWORD"), config.Config("PGDATABASE"))
