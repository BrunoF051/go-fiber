...
func ConnectDb() {
	...
	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s sslmode=require TimeZone=Eu/Central endpoint=%s", config.Config("PGHOST"), config.Config("PGUSER"), config.Config("PGPASSWORD"), config.Config("PGDATABASE"), config.Config("PGENDPOINT"))
